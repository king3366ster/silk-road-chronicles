 ...
 * Fixed: replaced alert/confirm with toast
 */
...
function _renderCounter(game, x, y, w, h) {
  ...
  types.forEach((t, i) => {
    game.ui.createText(x + 80 + i * colW, y, UNIT_TYPES[t]?.name || t, { fontSize: 10, fill: UNIT_TYPES[t]?.color || 0xCCCCCC });
  });
  y += 20;

  types.forEach(attacker => {
    game.ui.createText(x, y, UNIT_TYPES[attacker]?.name || attacker, { fontSize: 10, fill: UNIT_TYPES[attacker]?.color || 0xCCCCCC });
    types.forEach((defender, i) => {
      const mult = COUNTER_TABLE[attacker]?.[defender] || 1.0;
      let color = 0xCCCCCC;
      if (mult > 1.2) color = 0x27AE60;
      else if (mult > 1.0) color = 0x7DCEA0;
      else if (mult < 0.7) color = 0xE74C3C;
      else if (mult < 1.0) color = 0xF39C12;
  ...
  Object.entries(MERCENARY_TYPES || {}).forEach(([id, m]) => {
    ...
    game.ui.createButton(x + w - 160, y + 12, 130, 30, `雇佣 ${m.cost}金`, () => {
      if (state.hireMercenary(id, m.cost)) {
        ...
        game.ui.showToast(`雇佣了 ${m.name} x10！`);
        _renderMilitary(game);
      } else {
        game.ui.showToast('金币不足！');
      }
    }, { color: 0x8B0000 });
    y += 62;
  });
  ...
  units.forEach(u => {
    ...
    game.ui.createButton(x + w - 200, y + 8, 80, 28, '征召x10', () => {
      if (state.recruitTribeSoldiers(u.key, 10)) {
        game.ui.showToast(`征召了 ${u.name} x10！`);
        _renderMilitary(game);
      } else {
        game.ui.showToast('资源不足！');
      }
    }, { color: 0x2E4053 });
    game.ui.createButton(x + w - 110, y + 8, 80, 28, '征召x50', () => {
      if (state.recruitTribeSoldiers(u.key, 50)) {
        game.ui.showToast(`征召了 ${u.name} x50！`);
        _renderMilitary(game);
      } else {
        game.ui.showToast('资源不足！');
      }
...
function _renderWar(game, x, y, w, h) {
  const hasWarAbility = state.unlockedAbilities && state.unlockedAbilities.includes('war_declaration');
  ...
  nationKeys.forEach(nId => {
    ...
    const isAtWar = state.atWar && state.atWar.includes(nId);

    game.ui.createPanel(x, y, w - 20, 55, 0x1a0a04);
    game.ui.createText(x + 10, y + 5, `${nation.name}`, { fontSize: 13, fill: nc, bold: true });
    if (isAllFemaleNation && isAllFemaleNation(nId)) {
    ...
    if (attackCheck.total > 0) {
      game.ui.createText(x + 10, y + 38, `部落任务: ${attackCheck.completed}/${attackCheck.required}`,
    ...
    if (isAtWar) {
      game.ui.createButton(x + w - 120, y + 12, 90, 28, '⚔️交战中', () => {}, { color: 0x8B0000 });
    } else if (attackCheck.canAttack) {
      game.ui.createButton(x + w - 120, y + 12, 90, 28, '⚔️宣战!', () => {
        state.declareWar(nId);
        game.ui.showToast(`已向${nation.name}宣战！`);
        _renderMilitary(game);
      }, { color: 0xE74C3C });
    } else {
  ...
  if (state.atWar && state.atWar.length > 0) {
    y += 5;
